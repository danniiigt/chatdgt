...
export const Chats = {
  ...
  async getAllWithLastMessage(): Promise<ChatWithLastMessage[]> {
    ...
    const chatsWithLastMessage: ChatWithLastMessage[] = (chats || []).map(
      (chat: any) => {
  ...
  /**
   * Obtener chats por ID de usuario con paginación y búsqueda
   */
  async getByUserId(
    userId: string,
    options?: {
  ...
  /**
   * Contar chats por ID de usuario con búsqueda opcional
   */
