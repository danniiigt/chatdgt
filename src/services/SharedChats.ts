...
export const SharedChats = {
  ...
  async isValidToken(token: string): Promise<boolean> {
    ...
    const { data, error } = await supabase
      .rpc('is_shared_chat_valid', { token })
   ...
   */
  async getOrCreateForChat(chatId: string, expiresAt?: string): Promise<SharedChat> {
   ...
   */
  async setExpiration(chatId: string, expiresAt: string | null): Promise<SharedChat> {
    const supabase = createClientSupabase()
